...
| `CFG.BUS.UNT` | `8`     | `int unsigned` | Data unit width (in most cases it should be 8, the size of a byte).
| `CFG.BUS.ADR` | `32`    | `int unsigned` | Address bus width.
| `CFG.BUS.DAT` | `32`    | `int unsigned` | Data bus width.

| `CFG.BUS_BYT` | `DAT/UNT`           | `int unsigned` | Byte enable width is the number of unit widths fitting into the data width.
| `CFG_BUS_OFF` | `$clog2(CFG.BUS_BYT)`   | `int unsigned` | Maximum transfer logarithmic size, also width of 'off' (unit offset within data width) part of address.
| `BUS_SIZ` | `$clog2(CFG_BUS_OFF+1)` | `int unsigned` | Width of logarithmic size signal.
|===
...
32-bit is the default data bus width `BUS.DAT` and 4-bit is the default byte enable width `BUS_BYT`.
Byte enable width `BUS_BYT` is a calculated local parameter,
...
| `req.byt` | `BUS_BYT` | Byte enable/select. Only used in byte enable mode.
...
| peripheral   | `req.byt` |    `'1` | Access with less than the full width shall trigger an error.
...
| `CFG.BUS.CHN` | `COMMON_HALF_DUPLEX` | `tcb_bus_channel_t` | Channel configuration.
|===
...
=== PMA (physical memory attributes) parameters
...
| `CFG.BUS.MIN` | `int unsigned`     | `0`~`CFG_BUS_OFF`            | `0`          | Minimum transfer logarithmic size.
| `CFG.BUS.OFF` | `int unsigned`     | `0`~`CFG_BUS_OFF`            | `0`          | Number of LSB address bits (the offset of unit/byte inside the data bus) tied to zero.
| `CFG.BUS.ALN` | `int unsigned`     | `0`~`CFG_BUS_OFF`            | `CFG_BUS_OFF`    | Alignment width, number of least significant address bits which are zero.
| `CFG.BUS.MOD` | `tcb_bus_mode_t`   | `LOG_SIZE`/`BYTE_ENA`    | `LOG_SIZE`   | Data size/position mode.
| `CFG.BUS.ORD` | `tcb_bus_order_t`  | `DESCENDING`/`ASCENDING` | `DESCENDING` | Byte order (related to little/big endian systems).
|===
...
[%autowidth]
|===
| `MOD`      | `ORD`        | `CFG.BUS.DAT` | `CFG.BUS.MIN`   | `CFG.BUS.OFF`   | `BUS.ALN`   | `ndn`   | description

| `LOG_SIZE` | `ASCENDING`  | `32`      | `CFG_BUS_OFF=2` | `CFG_BUS_OFF=2` | `CFG_BUS_OFF=2` | ignored | RISC-V single issue instruction fetch unit without C extension.
...
| `LOG_SIZE` | `DESCENDING` | `32`/`64` | `0`         | `0`         | `CFG_BUS_OFF`   | ignored | RISC-V RV32/64 GPR load/store (no sign extension) with only aligned access support.
...
| `BYTE_ENA` | `DESCENDING` | `32`/`64` | `0`         | `CFG_BUS_OFF`   | `CFG_BUS_OFF`   | both    | RISC-V RV32/64 memory load/store with only aligned access support.

| `BYTE_ENA` | `DESCENDING` | `32`      | `CFG_BUS_OFF=2` | `CFG_BUS_OFF=2` | `CFG_BUS_OFF=2` | both    | Peripheral bus with only 4-byte (word) aligned access support.
| `BYTE_ENA` | `DESCENDING` | `64`      | `CFG_BUS_OFF=3` | `CFG_BUS_OFF=3` | `CFG_BUS_OFF=3` | both    | Peripheral bus with only 8-byte (double) aligned accesses support.
| `BYTE_ENA` | `DESCENDING` | `64`      |         `2` |         `2` | `CFG_BUS_OFF=3` | both    | Peripheral bus with mixed 4/8-byte aligned accesses support.
...
[%autowidth]
|===
| `BUS.DAT` | `BUS_BYT` | `$clog2(CFG.BUS_BYT) =CFG_BUS_OFF =    siz` | `$clog2(CFG_BUS_OFF+1) = BUS_SIZ` | comment
...
| `BUS.DAT` |      `32` |      `64` |
| `BUS_BYT` |       `4` |       `8` | `$clog2(CFG.BUS.DAT)`
| `CFG_BUS_OFF` |       `2` |       `3` | `$clog2(CFG.BUS_BYT)`
| `BUS_SIZ` |       `2` |       `2` | `$clog2(CFG_BUS_OFF+1)`
| `BUS.NDN` | `TCB_NDN_DEFAULT`     | Endianness should be matching the default for the given byte order (CFG.BUS.ORD parameter).

| `PMA.MIN` | `CFG_BUS_OFF` | `CFG_BUS_OFF` | Minimum transfer size is equal to maximum.
| `PMA.OFF` | `CFG_BUS_OFF` | `CFG_BUS_OFF` | Entire address offset must be zero (`CFG_BUS_OFF` LSB of address).
| `PMA.ALN` | `CFG_BUS_OFF` | `CFG_BUS_OFF` | Entire address offset must be aligned to transfer size (already covered by previous parameters).
|===
