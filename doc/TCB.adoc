...
| `PMA.MIN` | `BUS_MAX` | `BUS_MAX` | Minimum transfer size is equal to maximum.
| `PMA.OFF` | `BUS_MAX` | `BUS_MAX` | Entire address offset must be zero (`BUS_MAX` LSB of address).
| `PMA.ALN` | `BUS_MAX` | `BUS_MAX` | Entire address offset must be aligned to transfer size (already covered by previous parameters).
|===
