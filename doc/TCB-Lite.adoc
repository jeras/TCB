...
at `HSK.DLY` clock periods after the transfer `trn` is active.
...
| `CFG_BUS_MAX` | `$clog2(CFG.BUS_BYT)`   | `int unsigned` | Maximum transfer logarithmic size, also width of 'off' (unit offset within data width) part of address.
| `BUS_SIZ` | `$clog2(CFG_BUS_MAX+1)` | `int unsigned` | Width of logarithmic size signal.
|===
...
| `CFG.BUS.MIN` | `int unsigned`     | `0`~`CFG_BUS_MAX`            | `0`          | Minimum transfer logarithmic size.
| `CFG.BUS.OFF` | `int unsigned`     | `0`~`CFG_BUS_MAX`            | `0`          | Number of LSB address bits (the offset of unit/byte inside the data bus) tied to zero.
| `CFG.BUS.ALN` | `int unsigned`     | `0`~`CFG_BUS_MAX`            | `CFG_BUS_MAX`    | Alignment width, number of least significant address bits which are zero.
...
| `LOG_SIZE` | `ASCENDING`  | `32`      | `CFG_BUS_MAX=2` | `CFG_BUS_MAX=2` | `CFG_BUS_MAX=2` | ignored | RISC-V single issue instruction fetch unit without C extension.
...
| `LOG_SIZE` | `DESCENDING` | `32`/`64` | `0`         | `0`         | `CFG_BUS_MAX`   | ignored | RISC-V RV32/64 GPR load/store (no sign extension) with only aligned access support.
...
| `BYTE_ENA` | `DESCENDING` | `32`/`64` | `0`         | `CFG_BUS_MAX`   | `CFG_BUS_MAX`   | both    | RISC-V RV32/64 memory load/store with only aligned access support.

| `BYTE_ENA` | `DESCENDING` | `32`      | `CFG_BUS_MAX=2` | `CFG_BUS_MAX=2` | `CFG_BUS_MAX=2` | both    | Peripheral bus with only 4-byte (word) aligned access support.
| `BYTE_ENA` | `DESCENDING` | `64`      | `CFG_BUS_MAX=3` | `CFG_BUS_MAX=3` | `CFG_BUS_MAX=3` | both    | Peripheral bus with only 8-byte (double) aligned accesses support.
| `BYTE_ENA` | `DESCENDING` | `64`      |         `2` |         `2` | `CFG_BUS_MAX=3` | both    | Peripheral bus with mixed 4/8-byte aligned accesses support.
...
[%autowidth]
|===
| `BUS.DAT` | `BUS_BYT` | `$clog2(CFG.BUS_BYT) =CFG_BUS_MAX =    siz` | `$clog2(CFG_BUS_MAX+1) = BUS_SIZ` | comment
...
| `CFG_BUS_MAX` |       `2` |       `3` | `$clog2(CFG.BUS_BYT)`
| `BUS_SIZ` |       `2` |       `2` | `$clog2(CFG_BUS_MAX+1)`
| `BUS.NDN` | `TCB_NDN_DEFAULT`     | Endianness should be matching the default for the given byte order (CFG.BUS.ORD parameter).

| `PMA.MIN` | `CFG_BUS_MAX` | `CFG_BUS_MAX` | Minimum transfer size is equal to maximum.
| `PMA.OFF` | `CFG_BUS_MAX` | `CFG_BUS_MAX` | Entire address offset must be zero (`CFG_BUS_MAX` LSB of address).
| `PMA.ALN` | `CFG_BUS_MAX` | `CFG_BUS_MAX` | Entire address offset must be aligned to transfer size (already covered by previous parameters).
|===
