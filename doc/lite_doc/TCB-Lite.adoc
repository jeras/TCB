...
From the point of view of a single TCB-Lite interface:
- a **system controller** is driving clock and reset signals,
- a **manager** is driving the handshake valid and request bus
  while sampling the handshake ready and response bus,
- a **subordinate** is sampling the handshake valid and request bus
  while driving the handshake ready and response bus,
- a **monitor** is sampling the handshake valid/ready and request/response busses.

image::tcb_lite_manager_subordinate_monitor.svg[Manager, subordinate and monitor]

From the interconnect point of view:
- managers (driving requests) are connected to a module's subordinate interface port,
- subordinates (driving responses) are connected to a modules manager interface port.

image::tcb_lite_managers_interconnect_subordinates.svg[Managers, subordinates and the interconnect]
...
A few SRAM examples are analyzed in a separate
link::../MEMORIES.md[memories document].
...
| `CFG_BUS_OFF` | `$clog2(CFG_BUS_BYT)`   | `int unsigned` | Width of offset signals `off` (byte offset within data bus).
| `CFG_BUS_SIZ` | `$clog2(CFG_BUS_OFF+1)` | `int unsigned` | Width of logarithmic size signal.
|===
...
| `req.ato` | `5`           | Atomic access code (based ob RISC-V ISA).
...
| `1'b1` | `1'b1` | Read-modify-write or AMO access.
|===
...
.128-bit data bus vector
[source,SystemVerilog]
----
logic [15:0][7:0] dat;
logic       [2:0] siz;
logic [15:0]      ben;
logic       [2:0] off = adr[3:0];
----
...
[%autowidth]
|===
| `ndn`        | `siz`               | `off`               | description

| `1'b0`       | `siz==2'd2`         | `off[0]==1'b0`      | RISC-V with C extension instruction fetch.
| undefined    | `siz==CFG_BUS_OFF`  | `off=='d0`          | Full size, aligned.
| undefined    | `siz<=CFG_BUS_OFF`  | `off=='d0`          | Variable size, aligned to full size.
| unrestricted | `siz<=CFG_BUS_OFF`  | `off[siz-1:0]=='d0` | Variable size, aligned.
| unrestricted | `siz<=CFG_BUS_OFF`  | unrestricted        | Variable size, unaligned.
| unrestricted | `siz> CFG_BUS_OFF`  | `adr[siz-1:0]=='d0` | Double size, aligned.
| unrestricted | `siz> CFG_BUS_OFF`  | unrestricted        | Double size, unaligned.
| unrestricted | `siz==CFG_BUS_OFF`  | unrestricted        | Full size, unconventional alignment.
|===
...
* address aligned to 2-byte size (`off[0]=1'b0`),
* within a 32-bit data bus misaligned accesses are supported (`off[1]` is unrestricted).
...
==== Full data width size, aligned address

It is common to only allow full data bus width and aligned transfers
when accessing memory mapped peripherals.

This case would specify the following parameter values and signal restrictions:

* logarithmic size mode (`CFG.BUS.MOD=1'b0`),
* endianness should not be relevant when accessing memory mapped peripheral registers,
* transfer size must be the full data bus width (`siz=CFG_BUS_OFF= $clog2(CFG_BUS_BYT)=$clog2(CFG.BUS.DAT/8)`),
* address is aligned to the transfer size (`off=0`).

The following tables lists such transfers for 32/64-bit data buses.

.32-bit full data width size, aligned address
[%autowidth]
|===
| size | `off`  | `siz`  | `dat`
...
.64-bit full data width size, aligned address
[%autowidth]
|===
| size | `off`  | `siz`  | `dat`
...
==== Variable data width size, aligned address

When accessing memory mapped peripherals,
while keeping address alignment to the full bus width,
allowing access sizes smaller than the full data bus width
could reduce toggling of data signals and thus improve power consumption.

For example a peripheral might have a 32-bit TCB-Lite interface,
able to atomically access 32-bit registers,
but some registers 16/8 or less fields.

This case would specify the following parameter values and signal restrictions:

* logarithmic size mode (`CFG.BUS.MOD=1'b0`),
* endianness should not be relevant when accessing memory mapped peripheral registers,
* transfers size can be anything up to the data bus width (`siz<=CFG_BUS_OFF`),
* address is aligned to the data bus width (`off=0`).

The following tables lists such transfers for 32/64-bit data buses.

.32-bit variable data width size, aligned address
[%autowidth]
|===
| size | `off`  | `siz`  | `dat`
...
.64-bit variable data width size, aligned address
[%autowidth]
|===
| size   | `off`  | `siz`  | `dat`
| byte   | `3'd0` | `2'd0` | `{   ,   ,   ,   ,   ,   ,   ,[0]}`
...
* transfer size from byte to data bus width `siz<=$clog2(ALN)`,
...
I a conventional address map an `XLEN` sized register consumes `XLEN/8` address locations.

TCB-Lite does not strictly require the address map follow this convention.
...
| `CFG.PMA.MAX` | `int unsigned`     | `0`~`CFG_BUS_OFF` | `0`           | Maximum transfer logarithmic size.
| `CFG.PMA.MIN` | `int unsigned`     | `0`~`CFG_BUS_OFF` | `0`           | Minimum transfer logarithmic size.
| `CFG.PMA.OFF` | `int unsigned`     | `0`~`CFG_BUS_OFF` | `0`           | Number of LSB address bits (the offset of unit/byte inside the data bus) tied to zero.
| `CFG.PMA.ALN` | `int unsigned`     | `0`~`CFG_BUS_OFF` | `CFG_BUS_OFF` | Alignment width, number of least significant address bits which are zero.
|===
...
| `LOG_SIZE` | `32`/`64`     | `0`             | `0`             | `CFG_BUS_OFF`   | ignored | RISC-V RV32/64 GPR load/store (no sign extension) with only aligned access support.
| `BYTE_ENA` | `32`/`64`     | `0`             | `0`             | 0               | both    | RISC-V RV32/64 memory load/store with misaligned access support.
| `BYTE_ENA` | `32`/`64`     | `0`             | `CFG_BUS_OFF`   | `CFG_BUS_OFF`   | both    | RISC-V RV32/64 memory load/store with only aligned access support.
| `BYTE_ENA` | `32`          | `CFG_BUS_OFF=2` | `CFG_BUS_OFF=2` | `CFG_BUS_OFF=2` | both    | Peripheral bus with only 4-byte (word) aligned access support.
| `BYTE_ENA` | `64`          | `CFG_BUS_OFF=3` | `CFG_BUS_OFF=3` | `CFG_BUS_OFF=3` | both    | Peripheral bus with only 8-byte (double) aligned accesses support.
| `BYTE_ENA` | `64`          | `2`             | `2`             | `CFG_BUS_OFF=3` | both    | Peripheral bus with mixed 4/8-byte aligned accesses support.
|===
...
* `PMA.BND` =CFG_BUS_OFF + 1
  For example a 32-bit CPU with a TCB `BUS.DAT=32`` (`CFG_BUS_OFF=2``).
...
== Appendix C: Planed protocol features

Draft ideas for future versions of the protocol.
They are listed here, so they can be discussed before various incompatible implementations arise.
