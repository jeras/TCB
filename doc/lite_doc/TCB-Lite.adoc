...
where an error signal is combinationaly related to the request,
see appendix TODO.
====
...
In logarithmic size mode only `2**siz` bytes alligned with LSB of `wdt` are written.
...
Subordinate devices can also stall the transfer if red data is not ready yet,

Read data is available on `rdt` after a fixed delay of 1 clock cycle from the transfer.

NOTE: in contrast to many interconnect standards,
      TCB specifies the use of byte enable signals `siz and/or `ben` to
      enable or disable read from each byte.

image::tcb_lite_read.svg[Read transfer]

==== Read modify write transfer
